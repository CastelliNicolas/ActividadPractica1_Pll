...
AS
BEGIN
    SELECT f.NroFactura, f.Fecha, f.Cliente, p.Nombre, p.IdFormaPago FROM Factura f
	JOIN FormaPago p ON f.IdFormaPago = p.IdFormaPago
END
GO
...
AS
BEGIN
    SELECT f.*, p.* FROM Factura f
	JOIN FormaPago p ON p.IdFormaPago = f.IdFormaPago
